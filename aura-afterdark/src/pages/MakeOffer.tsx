    ...
    <div className="min-h-screen pt-6 pb-32 px-4 bg-aura-bg flex items-center justify-center">
      <div className="max-w-2xl w-full">
        ...
        <div className="bg-aura-surface/5 border border-aura-border rounded-3xl overflow-hidden">
          {/* Header */}
          <div className="bg-gradient-to-r from-aura-gold/20 to-orange-500/20 border-b border-aura-gold/30 p-8 text-center">
            <div className="text-7xl mb-4">💡</div>
            <h1 className="text-4xl font-bold text-aura-text mb-3">发起求购</h1>
            <p className="text-aura-text-secondary text-lg">向创作者表达你的购买意愿</p>
          </div>

          <form onSubmit={handleSubmit} className="p-8">
            {/* Content Preview */}
            <div className="mb-8 bg-aura-surface/5 rounded-2xl p-6">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-aura-text-secondary">@{post.author}</span>
                  </div>
                  <p className="text-sm text-aura-text-secondary line-clamp-2">{post.description}</p>
                </div>
              </div>
            </div>
            ...
            <div className="mb-6">
              <label className="block text-sm font-medium mb-2">
                出价金额 ($ORA) <span className="text-error">*</span>
              </label>
              <input
                type="number"
                value={offerAmount}
                onChange={(e) => setOfferAmount(e.target.value)}
                className="w-full px-4 py-4 bg-aura-surface/5 border border-aura-border rounded-xl focus:outline-none focus:border-aura-purple transition-colors text-aura-text text-2xl font-bold"
                placeholder="输入你的出价..."
                min="1"
                step="1"
                required
              ...
              <div className="flex gap-2 mt-3">
                {[10, 50, 100, 500, 1000].map((amount) => (
                  <button
                    ...
                    className="flex-1 py-2 bg-aura-surface/10 rounded-lg text-sm hover:bg-aura-surface/20 transition-colors"
                  >
                    {amount}
                  </button>
                ))}
            ...
            <div className="mb-6">
              ...
              <textarea
                ...
                rows={4}
                maxLength={500}
                className="w-full px-4 py-3 bg-aura-surface/5 border border-aura-border rounded-xl focus:outline-none focus:border-aura-purple transition-colors text-aura-text resize-none"
                placeholder="说明你的购买意愿、用途等..."
              />
              <div className="text-xs text-aura-text-secondary text-right mt-1">
                {message.length} / 500
              </div>
            </div>

            {/* Rules */}
            <div className="bg-aura-gold/10 border border-aura-gold/30 rounded-xl p-6 mb-6">
              <h3 className="text-aura-gold font-semibold mb-3 flex items-center gap-2">
                <span>📝</span>
                <span>求购规则</span>
              </h3>
              ...
              <div className="bg-aura-surface/5 rounded-xl p-6 mb-6">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    <span className="text-aura-text-secondary">出价金额</span>
                    <span className="text-xl font-bold">{offerAmount} $ORA</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-aura-text-secondary">托管费用</span>
                    <span className="text-success">免费</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-aura-text-secondary">若接受，创作者收益</span>
                    <span>{(parseFloat(offerAmount) * 0.95).toFixed(2)} $ORA (95%)</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-aura-text-secondary">若接受，平台手续费</span>
                    <span>{(parseFloat(offerAmount) * 0.05).toFixed(2)} $ORA (5%)</span>
                  </div>
                  <div className="border-t border-aura-border pt-3 flex justify-between">
                    <span className="font-bold">你需要支付</span>
                    <span className="text-2xl font-bold text-aura-gold">{offerAmount} $ORA</span>
                  </div>
                </div>
              </div>
            )}
