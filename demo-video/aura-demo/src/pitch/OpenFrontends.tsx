/**
 * Act 5 · Open Frontends (1:26 – 1:40, 14s)
 *
 ...
 *
 * Beat:
 *   0–2.5s  "Anyone can build a frontend."
 *   2.5–11s  Three frontend examples (Mainstream / AfterDark / Anyone)
 *            connect to a single AURA Protocol core
 *  11–14s   Closing: "Protocol stays open. Frontends stay accountable."
 */
...
export const OpenFrontends: React.FC = () => {
  ...
  const headlineIn = interpolate(frame, [0, sec(0.5)], [0, 1], { extrapolateRight: 'clamp' });
  const headlineOut = interpolate(frame, [sec(10), sec(10.7)], [1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
  const headline = Math.min(headlineIn, headlineOut);

  const closingIn = interpolate(frame, [sec(10.7), sec(11.4)], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
    ...
    <AbsoluteFill style={{ background: bgGradient(), fontFamily: FONT.family, color: COLORS.fg, padding: '60px 100px' }}>
      ...
      {headline > 0 && (
        <>
          ...
          >
            {frontends.map((f, i) => {
              const start = sec(0.9 + i * 0.35);
              const enter = spring({ frame: frame - start, fps, config: { damping: 16, stiffness: 105 } });
              return (
                <div
                  key={f.name}
                  style={{
